...
main = do
    ...
    writeFile "output.txt" . concat . zipWith (\x y -> show x ++ " predicted to be " ++ show y ++ "\n") (tail input) . neuronOutput (trainWhile neuron (prepareInputs input) (tail input)) $ prepareInputs input
